...
/* $Id: Smarty_Compiler.class.php,v 1.1 2009/09/04 13:44:49 tihonove Exp $ */
...
class Smarty_Compiler extends Smarty {
		...
		switch ( $tag_command) {
			...
			case 'else' :
				...
				if ($_open_tag != 'if' && $_open_tag != 'elseif')
					$this->_syntax_error ( 'unexpected {else}', E_USER_ERROR, __FILE__, __LINE__ ); else
					$this->_push_tag ( 'else' );
				return '<?php else: ?>';
			...
			case '/foreach' :
				...
				if ($_open_tag == 'foreachelse')
					return "<?php endif; unset(\$_from); ?>"; else
	...
	function _compile_block_tag($tag_command, $tag_args, $tag_modifier, &$output) {
		...
		if ($start_tag)
			$this->_push_tag ( $tag_command ); else
			...
			switch ( $attr_name) {
				...
				case 'show' :
					if (is_bool ( $attr_value ))
						$show_attr_value = $attr_value ? 'true' : 'false'; else
	...
	function _compile_if_tag($tag_args, $elseif = false) {
		...
		for($i = 0; $i < count ( $tokens ); $i ++) {
			...
			switch ( strtolower ( $token )) {
				...
				case 'is':
                    ...
                    if ($tokens [$i - 1] == ')')
						$is_arg_start = array_pop ( $is_arg_stack ); else
						$is_arg_start = $i - 1;
		...
		if ($elseif)
			return '<?php elseif (' . implode ( ' ', $tokens ) . '): ?>'; else
			...
			foreach ( $_indexes as $_index ) {
				if (substr ( $_index, 0, 1 ) == '[') {
					...
					if (substr ( $_index, 1, 1 ) == '$')
						$_output .= "[\$this->_tpl_vars['" . substr ( $_index, 2 ) . "']]"; else
	...
	function _parse_modifiers(&$output, $modifier_string) {
		...
		for($_i = 0, $_for_max = count ( $_modifiers ); $_i < $_for_max; $_i ++) {
			...
			if (count ( $_modifier_args ) > 0)
				$_modifier_args = ', ' . implode ( ', ', $_modifier_args ); else
				$_modifier_args = '';
