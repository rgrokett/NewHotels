...
abstract class Dataset
{
    ...
    function Post()
    {
        ...
        elseif ($this->insertMode)
        {
            ...
            foreach($this->insertFieldValues as $fieldName => $value)
            {
                ...
                {
                //echo $fieldName . ':' .($this->GetFieldByName($fieldName)->GetIsAutoincrement() ? 'true' : 'false') .', ';
                    if ($this->GetFieldByName($fieldName)->GetIsAutoincrement())
    ...
    public function GetPrimaryKeyFieldNames() { return $this->primaryKeyFields; }

    /* public function GetPrimaryKeyFieldCount() { return count($this->primaryKeyFields); }
    public function GetPrimaryKeyValue($index)
    {
        $primaryKeyFields = $this->GetPrimaryKeyFieldNames();
        return $this->GetFieldValueByName($primaryKeyFields[$index]);
    }
    */
    ...
    function SetMasterFieldValue($fieldName, $value) { $this->masterFieldValue[$fieldName] = $value; }
    
    function GetMasterFieldValueByName($fieldName)
    {
        return GetArrayValueDef($this->masterFieldValue, $fieldName);
        
        /* if (isset($this->masterFieldValue[$fieldName]))
            return $this->masterFieldValue[$fieldName];
        else
            return null; */
    }
