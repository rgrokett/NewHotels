...
<div align="center" style="width: 100%">
    <form name="editform" enctype="multipart/form-data" method="POST" action="{$Grid->GetEditPageAction()}">
...
{/foreach}
        <table class="grid" style="width: auto">
            ...
            <tr><td class="odd grid_error_row" colspan=4>
                ...
                <strong>{$Captions->GetMessageString('ErrorsDuringUpdateProcess')}</strong><br><br>
...
{if $column->GetAllowSetToDefault()}
    <input type="checkbox" value="1" name="{$column->GetFieldName()}_def" id="{$column->GetFieldName()}_def"/>{$Captions->GetMessageString('SetDefault')}</div>
{/if}
            </td>
            </tr>
{/foreach}
            ...
            <tr height="40" class="editor_buttons"><td colspan="4" align="center" valign="top" >
                <input class="sm_button" type="button" value="{$Captions->GetMessageString('Save')}" name="submit1" onclick="if (ValidateControls()) document.editform.submit();"/>
